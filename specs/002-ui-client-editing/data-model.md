...
### AsciiDoc files
- `infra/<product>/preamble.adoc` and `suffix.adoc` — product-level
- `infra/<product>/<size>/<flavour>/preamble.adoc` and `suffix.adoc` — flavour-level
...
├── preamble_content: str     # full file content; empty string if file absent
├── suffix_content: str
├── preamble_change: ChangeState
