...
- [X] T019 [US1] Implement `render_product_document(product, build_date)` in `src/renderer.py` — assembles full document: header + `include::infra/prefix.adoc[]` + `include::infra/{shortname}/prefix.adoc[]` + size sections (pass `is_single_size=len(sizes)==1`) + `include::infra/{shortname}/suffix.adoc[]` + `include::infra/suffix.adoc[]`; all paths relative to repo root (resolved via `-B` flag) (FR-009, FR-010, FR-011, FR-013a)
...
- [X] T026 [US2] Validate `render_product_document()` embeds the correct common prefix/suffix includes and that product-specific prefix/suffix includes differ per product — assert generated `.adoc` content in `tests/unit/test_renderer.py` (FR-010, FR-011)
- [X] T027 [US2] Validate mandated section order in generated `.adoc` for multi-product build: `header → include::infra/prefix.adoc[] → include::infra/{p}/prefix.adoc[] → sizes → include::infra/{p}/suffix.adoc[] → include::infra/suffix.adoc[]` — assert in `tests/unit/test_renderer.py` (FR-009, SC-002)
