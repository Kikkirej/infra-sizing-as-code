...
def _flavour_tree(f) -> dict:
    return {
        ...
        "change": f.change,
        "has_prefix": bool(f.prefix_content),
        "has_suffix": bool(f.suffix_content),
        "prefix_change": f.prefix_change, "suffix_change": f.suffix_change,
    ...
    for sn, p in state.products.items():
        ...
        for ssn, s in p.sizes.items():
            ...
            sizes_out[ssn] = {
                ...
                "change": s.change,
                "has_prefix": bool(s.prefix_content),
                "has_suffix": bool(s.suffix_content),
                "prefix_change": s.prefix_change, "suffix_change": s.suffix_change,
                "flavours": flavours_out,
            }
        products_out[sn] = {
            "shortname": p.shortname, "display_name": p.display_name,
            "change": p.change, "error": p.error,
            "has_prefix": bool(p.prefix_content),
            "has_suffix": bool(p.suffix_content),
            "prefix_change": p.prefix_change, "suffix_change": p.suffix_change,
            "sizes": sizes_out,
        }
    return {
        "globals": {
            "prefix_change": state.global_prefix_change,
