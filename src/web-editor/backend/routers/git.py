...
def git_changes():
    ...
    for p in state.products.values():
        ...
        if p.preamble_change != ChangeState.CLEAN:
            changes.append(f"infra/{p.shortname}/preamble.adoc")
        ...
        for s in p.sizes.values():
            ...
            for f in s.flavours.values():
                ...
                if f.preamble_change != ChangeState.CLEAN:
                    changes.append(f"infra/{p.shortname}/{s.shortname}/{f.shortname}/preamble.adoc")
