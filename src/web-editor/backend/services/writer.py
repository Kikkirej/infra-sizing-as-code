...
def write_all(state: EditorState, repo_root: Path) -> None:
    ...
    for product in state.products.values():
        ...
        if product.change in (ChangeState.ADDED, ChangeState.MODIFIED):
            atomic_write_json(product_dir / "meta.json", {"preamble": "preamble.adoc", "suffix": "suffix.adoc"})

        if product.preamble_change != ChangeState.CLEAN:
            atomic_write_text(product_dir / "preamble.adoc", product.preamble_content)
        ...
        for size in product.sizes.values():
            ...
            if size.change in (ChangeState.ADDED, ChangeState.MODIFIED):
                atomic_write_json(size_dir / "meta.json", {
                    "prefix_text": size.prefix_text,
                    "suffix_text": size.suffix_text,
                })
            ...
            for flavour in size.flavours.values():
                ...
                if flavour.preamble_change != ChangeState.CLEAN:
                    atomic_write_text(flavour_dir / "preamble.adoc", flavour.preamble_content)
