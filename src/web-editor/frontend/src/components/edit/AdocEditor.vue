<template>
  <div class="adoc-editor">
    ...
    <div v-else class="adoc-preview">
      <div v-if="renderError" class="adoc-render-error">Render error: {{ renderError }}</div>
      <div v-html="rendered" />
    </div>
  </div>
</template>
...
const rendered = computed(() => {
  try {
    ...
    return asciidoctor.convert(content.value || '', { safe: 'safe', attributes: { showtitle: '' } })
  } catch (e) {
...
.adoc-render-error { padding: 8px 12px; margin-bottom: 8px; background: #fee2e2; color: #dc2626; border-radius: 4px; font-size: 12px; }
</style>
